...
Basically appears to divide the total releases by the tagged, of marked?
...
Bigger question - appears to calc summaries on StockID/PR_MU rather than tag_code level...
...
rmis_rr |> 
  ...
  #count(cwt_1st_mark, cwt_2nd_mark)
#filter(Run_ID==2020, PR_MU=="1_4") |> select(PR_MU, tag_code, cwt_1st_mark:FisheryID)
#filter(Run_ID==2020, PR_MU=="1_3") |> select(PR_MU, tag_code, cwt_1st_mark:FisheryID)
  group_by(
    Run_ID, StockID, PR_MU, tag_code
    ) |> 
  #mutate( 
  summarise(
...
read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/RRTerm_Automated_DataInputs/MU_Local_PEF_010422rv.csv") |> 
  filter(Run_ID == 2020)
