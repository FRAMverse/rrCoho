...
#do not appear to actually have original raw TOCAS data: mbellman_PostSeasCatchInputFina_12_16_2021.csv
...
#short term fix? coerce to NA everything not currently in unique(lu_coho_tocas_fram$Disposition)  
#better to explicitly comma-separate allowed dispositions in lu_coho/lu_coho_tocas_fram
#then separate_rows from allowed set: 'COMM','C&SF', etc.
#and that then fixes the bandaid "!TEST"
#However, TribeName may need this soln? unique(lu_coho_tocas_fram$TribeName)
  ...
  left_join(
    lu$tocas_fram |> separate_rows(Gear) |> mutate(Gear = as.integer(Gear)),
