...
```{r escp_hvst_cwt_pef}
#per AHB, replicating past practice
#only considering HR_Flag==1 and PR 1:6
#so, filtering at outset after examining equivalent effect of later
#but PR_MU 5_24 SPGSND foxicc is example of potential consideration
#PR_MU allowed in CA is excluded when no escapement compiled

escp_hvst_cwt_pef <- lu$ca_pr_mu |> 
  full_join(
    data_in$escp |> 
      select(year, PR_MU, escp)
    ,
...
vector of included sport fisheries, designated by river or not
move this into the `lu$fram_ca`?

```{r ca_spt}
ca_spt <- lu$ca_pr_mu |> 
  filter(PR_Number < 7, str_detect(CA_Short_Name, "pt$")) |> 
  distinct(CA_Number, CA_Short_Name) |> 
  mutate(river = if_else(str_detect(CA_Short_Name, "R Spt$"), T, F))
```
