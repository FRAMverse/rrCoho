...
mb20 <- list()
mb20$escp <- read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/Data_ProcessingSteps/Escapement_Inputs/MU_Escapement_013122.csv")
mb20$crc <- read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/Data_ProcessingSteps/Landings_Sport/RRTerm_CA_Catch_FinalInputSpt_RegularTS_012722.csv")
mb20$tocas <- read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/Data_ProcessingSteps/Landings_Commercial/RRTerm_CA_Catch_CommFinalwNames_RegularTS_121621.csv")

#only a few years, mostly 2019-2020, as raw files
#still silly big number of columns
mb20$cwt_raw <- left_join(
  readr::read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/Data_ProcessingSteps/Raw_RMIS_CWTQueryData/Recovery_Data_121621/CSV6559.csv")
  ,
  readr::read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/Data_ProcessingSteps/Raw_RMIS_CWTQueryData/Release_Data_121621/CSV6205.csv") |> 
    select(
  ...
  #or: "T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/Data_ProcessingSteps/Raw_RMIS_CWTQueryData/AllRelease_Data_123021/CSV1601.csv"
  ,
...
mb20$pef <- read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/RRTerm_Automated_DataInputs/MU_Local_PEF_010422rv.csv") 
...
#already filtered to only marked cwt_1st (5000:6000)
data_in$pef <- rmis_rr |> 
  distinct(tag_code, .keep_all = T) |> 
  filter(last_release_date_year - brood_year == 2) |> #no effect in this test dataset
  mutate(
    Run_ID = brood_year + 3,
    across(ends_with("_count"), ~replace_na(.,0)),
    rel_tot = cwt_1st_mark_count + cwt_2nd_mark_count + non_cwt_1st_mark_count + non_cwt_2nd_mark_count,
    rel_m_cwt_1st = if_else(between(as.integer(cwt_1st_mark), 5000, 6000), cwt_1st_mark_count, 0),
    rel_m_cwt_2nd = if_else(between(as.integer(cwt_2nd_mark), 5000, 6000), cwt_2nd_mark_count, 0),
    across(starts_with("rel_m_cwt"), ~replace_na(.,0))
    ) |> 
  group_by(
    year = Run_ID, StockID, PR_MU
    #, tag_code
    ) |> 
  summarise(
    across(starts_with("rel_"), sum),
    pef = rel_tot / rel_m_cwt_1st + rel_m_cwt_2nd,
    .groups = "drop"
  ) 
```

```{r compare_data_in_pef}
#small mismatches look due to excluding UM+tagged
#large mismatches look due to missing large portion of release
#likely need to start from "all_releases" rather than only from recovered tag_codes?

data_in$pef |> 
  filter(year==2020) |> 
  full_join(
    #object export with "_Num" and "_Den" fields
    read_csv("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/MB_Draft_2020/Data_ProcessingSteps/PEF_Method_Files/LocalPEF_Match5sum_010422.csv") |> 
      filter(Run_ID == 2020) |>
      drop_na(Local_PEF) |> 
      select(Run_ID, StockID = FRAM_StockID, Local_PEF_Num:Local_PEF)
    ,
    by = c("year" = "Run_ID", "StockID")
  ) |> 
  left_join(
    mb20$pef |> 
      unite("PR_MU", PR_Number, PR_MU_Number) |> 
      select(Run_ID, PR_MU, MU_Short_Name, Local_PEF_mdb = Local_PEF)
    ,
    by = c("Run_ID", "PR_MU")
  ) |> 
  print(n=50)
...
hvst_cwt_pef_escp <- bind_rows(
  ...
  left_join(
    data_in$pef |> 
      select(year, PR_MU, pef)
    ,
    by = c("year", "PR_MU")
  ) |> 
  mutate(
    cwt_pef = est_num * pef
  ) |> #filter(!is.na(cwt_pef)) |> print(n=100)
...
#NEXT FINISH FROM HERE
  ...
  group_by(year, CA_Number) |> 
  mutate(
    escp = if_else(is.na(HR_Flag), 0, escp),
