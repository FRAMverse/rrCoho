...
##for 2020 testing in PR1, to avoid negative MB notes show she simply set Lummi P flag to 0
  mutate(
    cwt_pef = if_else(CA_Number == 531 & PR_MU == "1_4", 0, cwt_pef)
  ) 
...
for (pr_num in 1:6){
  ...
  #calc first fishery/CA outside of for-loop
  pr_temp <- pr_ca_mu |> 
...
## delete when ready: initial version on idea of separated sections/chunks per PRs 

```{r}
#not starting from escp_hvst_cwt_pef
pr_num <- 1

#potential PR_MUs in relevant CAs
pr_ca_mu <- lu$ca_pr_mu |> 
  filter(HR_Flag == 1, PR_Number == pr_num) |> 
  select(CA_Number, CA_Short_Name, PR_MU, MU_Short_Name)

#order of fisheries to evaluate
ca_seq <- sort(unique(pr_ca_mu$CA_Number), decreasing = T)
...
pr_catch_less_cwt <- pr_catch |> #filter(year==2020)
  inner_join(
    pr_cwt_pef |> select(year, CA_Number, PR_MU, cwt_pef),
    by = c("year", "CA_Number")
  ) |> 
  filter(CWT_Flag > 0) |> 
  group_by(year, CA_Number) |> 
...
#first fishery/CA outside of for-loop
pr_temp <- pr_ca_mu |> 
    filter(CA_Number == ca_seq[1]) |> 
    left_join(pr_run_size, by = c("PR_MU")) |> 
    left_join(pr_catch, by = c("year", "CA_Number")) |> 
    left_join(pr_cwt_pef |> select(year, CA_Number, PR_MU, cwt_pef), by = c("year", "CA_Number", "PR_MU")) |> 
    left_join(pr_catch_less_cwt, by = c("year", "CA_Number")) |> 
...
pr_out <- pr_temp |> select(CA_Number:year, catch_pr_mu)

#now process remaining fisheries/CAs
#using starting run size as either escapement or that calc'd from previous fishery
#and splitting catch-less-CWT by relative proportions of those MUs without cwt-based catch
for (ca in ca_seq[-1]) {
  pr_temp <- pr_ca_mu |> 
    filter(CA_Number == ca) |> 
    left_join(pr_run_size, by = c("PR_MU")) |> 
    left_join(pr_catch, by = c("year", "CA_Number")) |> 
    left_join(pr_cwt_pef |> select(year, CA_Number, PR_MU, cwt_pef), by = c("year", "CA_Number", "PR_MU")) |> 
    left_join(pr_catch_less_cwt, by = c("year", "CA_Number")) |> #filter(year==2020)
    group_by(year) |> 
    mutate(
      rs_tot = sum(rs[is.na(cwt_pef)]),
      rs_pct = if_else(is.na(cwt_pef), rs / rs_tot, 0),
...
pr_out <- bind_rows(
  pr_out,
  pr_temp |> select(CA_Number:year, catch_pr_mu)
)
