...
params:
  ...
  file_path_crc: "T:/DFW-Salmon Mgmt Modeling Team - General/Catch datasets/CRC/Sport Harvest Estimates 20221116.mdb"
  file_path_tocas: "T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/tocas_coho_2000-present.csv"
  file_path_rmis_rec_rel: "mb20$cwt_raw"
  file_path_rmis_all_rel: "rmis_releases_all_coho_by2010_onward.csv"
editor_options: 
...
#lookups from CRC, TOCAS, and RMIS to FRAM FisheryID and RMIS to PR_MU/StockID
lu_xlsx <- "~/O/code/framr/xlsx/lu_coho.xlsx"
lu_xlsx_sheets <- set_names(readxl::excel_sheets(lu_xlsx))
lu <- map(lu_xlsx_sheets, ~readxl::read_excel(lu_xlsx, sheet = .x))
...
```{r lu_coho_xlsx}
# #moving to single xlsx of LUs; overwriting original framr version with only catch-to-FRAM-FisheryID
# #initial version, passed to AHB for manual reflagging of ca_pr_mu
# writexl::write_xlsx(lu, "lu_coho.xlsx")
# #corrected a few H+W in NS net CAs
# #added MSF_Flag column and blank/NAs to remove W from MSF CA
...
Persisted CRC~FRAM LU as assigned. Migrated assignment to `framr`-based LU 

Then assign RRTerm CA from FRAM FisheryID

Catches only summed from FRAM timestep 3+4+5 FW or 4+5 Marine (Aug+Sep+Oct-Nov).

Seems undesirable, but persisting per AHB. Apparently "dip-in" concerns.
...
data_in$crc_ca <- data_in$crc |> 
  ...
  drop_na(RRTermType) |> 
  filter(ts > 2) |> 
  group_by(year, CA_Number, CA_Short_Name, 
           #prior to moving to CA_PR_MU table: #CWT_Flag, Cat_Flag, Type_Calc,
           RRTermType, ts) |> 
  summarise(catchest = sum(catchest), .groups = "drop") |> 
...
```{r rmis_rec_rel_preliminary_testing}
#unmapped recoveries
...
data_in$cwt <- rmis_rec_rel |> 
  ...
  group_by(
    year = rec_year, ts,
    CA_Number, CA_Short_Name, 
    #prior to moving to CA_PR_MU table: #CWT_Flag, Cat_Flag, Type_Calc,
    RRTermType, PR_Number, PR_MU_Number, PR_MU, MU_Short_Name) |> 
...
data_in$cwt |> 
  ...
  filter(abs(est_num - CWT_Est_Rec) > 1) |> 
  print(n=100)
...
rr$cwt_pef <- data_in$cwt |> 
  ...
  mutate(cwt_pef = est_num * pef_adcwt)
...
## prior to `lu$ca_pr_mu_flagged` with HR_Flag, CWT_Flag and (new) MSF_Flag on single stock-in-fishery table

```{r shared_intermediates}
#potential PR_MUs in relevant CAs
pr_ca_mu <- lu$ca_pr_mu |> 
  filter(HR_Flag == 1, PR_Number < 7) |> 
  select(PR_Number, CA_Number, CA_Short_Name, PR_MU, MU_Short_Name)

#expanded estimated_number values
pr_cwt_pef <- data_in$cwt |> 
  select(year, CA_Number, PR_MU, est_num = estimated_number) |> 
  semi_join(pr_ca_mu, by = c("CA_Number", "PR_MU")) |> 
  left_join(data_in$pef |> select(year, PR_MU, pef_adcwt), by = c("year", "PR_MU")) |> 
  mutate(cwt_pef = est_num * pef_adcwt)
  ##mutate(cwt_pef = if_else(CA_Number == 531 & PR_MU == "1_4", 0, cwt_pef))

##for 2020 testing, MB notes show she did not use CWTs in these
pr_cwt_pef <- pr_cwt_pef |> 
  filter(!(CA_Number %in% c(531, 537)))


#original catch values before deducting expanded CWT-catch
pr_catch <- data_in$catch |>
  select(year, CA_Number, CWT_Flag, catch) |>
  semi_join(pr_ca_mu, by = c("CA_Number"))
##for 2020 testing against MB
pr_catch <- pr_catch |> 
  mutate(catch = if_else(year ==2020 & CA_Number == 563, 13357, catch))

#catch values after deducting total CWT-catch summed across PR_MUs
pr_catch_less_cwt <- pr_catch |> 
##for 2020 testing in PR1, to avoid negatives MB notes show she reflagged to 0
  mutate(CWT_Flag = if_else(CA_Number %in% c(537, 531), 0, CWT_Flag)) |> 
  filter(CWT_Flag > 0) |> 
  inner_join(pr_cwt_pef |> select(year, CA_Number, PR_MU, cwt_pef), by = c("year", "CA_Number")) |> 
  group_by(year, CA_Number) |> 
  summarise(catch_less_cwt = catch[1] - sum(cwt_pef), .groups = 'drop') 

#relevant escapement
pr_escp <- data_in$escp |> 
  select(year, PR_MU, escp) |> 
  semi_join(pr_ca_mu, by = c("PR_MU"))
...
```{r}
pr_catch <- pr_catch |> filter(year==2020)
pr_catch_less_cwt <- pr_catch_less_cwt |> filter(year==2020)
pr_cwt_pef <- pr_cwt_pef |> filter(year==2020)
pr_run_size <- pr_run_size |> filter(year==2020)
```
...
for (pr_num in 1:6){
  ...
  pr_temp <- pr_ca_mu |> 
    filter(PR_Number == pr_num, CA_Number == ca_seq[1]) |> 
    left_join(pr_run_size, by = c("PR_MU")) |> 
    left_join(pr_catch, by = c("year", "CA_Number")) |> 
    left_join(pr_cwt_pef |> select(year, CA_Number, PR_MU, cwt_pef), by = c("year", "CA_Number", "PR_MU")) |> 
    left_join(pr_catch_less_cwt, by = c("year", "CA_Number")) |> 
    group_by(PR_Number, CA_Number, year) |> 
    mutate(
      rs_tot = sum(rs[is.na(cwt_pef)]),
      rs_pct = if_else(is.na(cwt_pef), rs / rs_tot, 0),
      catch_less_cwt = if_else(is.na(catch_less_cwt), catch, catch_less_cwt),
      catch_pr_mu = if_else(
        is.na(cwt_pef),
        catch_less_cwt * rs_pct,
        cwt_pef
        ) |> 
        replace_na(0),
      rs = rs + catch_pr_mu
  ...
  pr_out <- rows_upsert(
    pr_out,
    pr_temp |> select(PR_Number:year, catch_pr_mu),
    by = c("PR_Number","CA_Number","CA_Short_Name","PR_MU","MU_Short_Name","year"))
...
compare against *PScohoRR_TRS_2010-2020_Updated_2022-01-31_wHoodCanal.xlsx*
and notes in *CohoRunReconstruction_2020LoadInputs_MB011222.docx*
first inspection looks like CWT_Flag diffs are main driver of meaningful result diffs
especially where cwt_pef values were manually zeroed by MB within GUI/EXE?

```{r init_QAQC}
pr_out |> 
  arrange(PR_Number, CA_Number, as.integer(str_sub(PR_MU, 3, 10))) |> 
  unite("ca", starts_with("CA_")) |> 
  filter(year==2020) |> 
  split(~PR_Number) |> 
  map(
  ...
  writexl::write_xlsx("T:/DFW-Salmon Mgmt Modeling Team - General/PS Coho Run Reconstruction/rrterm/temp_qaqc_2020.xlsx")
```
